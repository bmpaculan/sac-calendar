(function(){
  const TAG = 'com-example-time-calendar-1-x'; // <— must match manifest "tag"
  ...
  tpl.innerHTML = `
    <style>
      *, *::before, *::after { box-sizing: border-box; }
      ...
      .empty{padding:10px;color:#6b7280;font-size:.9rem}
    </style>
    <div class="wrap">
  ...
  async function loadDaysFromBinding(db){
    if (!db) return {};
    try {
      if (typeof db.getData === 'function') {
        const payload = await db.getData();
        const rows = findRowsBlock(payload);
        const dims = payload.dimensions || payload?.data?.dimensions || db.dimensions || null;
        return rowsToDays(rows, dims);
      }
    } catch(e){
      console.warn('getData() failed; fallback to static payload', e);
    }
    const rows = findRowsBlock(db);
    const dims = db.dimensions || db?.data?.dimensions || null;
    return rowsToDays(rows, dims);
  }
  ...
  class SacCalendar extends HTMLElement {
    ...
    onCustomWidgetAfterUpdate(){
      ...
      if (!db) { this._clear(); return; }

      // 1) initial load
      this._fetchAndRender(db);

      // 2) listen for changes (mapping changes, story filter changes, etc.)
      if (typeof db.attachDataChanged === 'function' && !this._listening) {
        this._listening = true;
        db.attachDataChanged(() => this._fetchAndRender(db));
      }
    }

    _clear(){ this.$legend.innerHTML=""; this.$calendars.innerHTML='<div class="empty">Bind Date and Status to render.</div>'; }
    ...
    _fetchAndRender(db){
      ...
      loadDaysFromBinding(db).then(daysMap=>{
        ...
        this._loading = false;
        console.error('loadDaysFromBinding error', err);
        this._clear();
      });
    }
    ...
    _render(){
      ...
      const months = (function(){
        const ks=Object.keys(daysMap).sort(); if(!ks.length) return [];
        return rangeMonths(ks[0].slice(0,7), ks[ks.length-1].slice(0,7));
      })();
  ...
  if (!customElements.get(TAG)) {
    customElements.define(TAG, SacCalendar);
  }
})();
